<template>
	<div class="main-content">
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"flexWrap":"wrap","justifyContent":"center","display":"flex"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#333","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">乘车点</label>
						<el-input v-model="searchForm.chengchedian" placeholder="乘车点" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#333","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">目的地</label>
				...
				<el-row :style='{"margin":"20px 0","display":"flex"}'>
					<el-button :style='{"border":"1px solid #999","cursor":"pointer","padding":"0 12px","boxShadow":"inset 0px 0px 32px 0px #eee","margin":"0 10px 0 0","color":"#000","minWidth":"80px","outline":"none","borderRadius":"4px","background":"linear-gradient(320deg, rgba(187,194,208,1) 0%, rgba(247,249,251,1) 50%, rgba(179,187,203,1) 100%)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('dingdanxinxi','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"1px solid #eb8787","cursor":"pointer","padding":"0 12px","boxShadow":"inset 0px 0px 32px 0px #f0d2d2","margin":"0 10px 0 0","color":"#c00","minWidth":"80px","outline":"none","borderRadius":"4px","background":"linear-gradient(240deg, rgba(255,168,153,1) 0%, rgba(247,249,251,1) 50%, rgba(249,156,141,1) 100%)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('dingdanxinxi','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid #7fbeef","cursor":"pointer","padding":"0 16px","margin":"3px 6px 3px 0","outline":"none","color":"#1e98f5","borderRadius":"4px","background":"linear-gradient(150deg, rgba(218,239,255,1) 0%, rgba(247,249,251,1) 50%, rgba(214,237,255,1) 100%)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('dingdanxinxi','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"1px solid #7fbeef","cursor":"pointer","padding":"0 16px","margin":"3px 6px 3px 0","outline":"none","color":"#1e98f5","borderRadius":"4px","background":"linear-gradient(150deg, rgba(218,239,255,1) 0%, rgba(247,249,251,1) 50%, rgba(214,237,255,1) 100%)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('dingdanxinxi','投诉')" type="success" size="mini" @click="wentidingdanCrossAddOrUpdateHandler(scope.row,'cross','','','')">投诉</el-button>
							<el-button :style='{"border":"1px solid #60e669","cursor":"pointer","padding":"0 16px","margin":"3px 6px 3px 0","outline":"none","color":"#3CB371","borderRadius":"4px","background":"linear-gradient(240deg, rgba(212,254,215,1) 0%, rgba(249,252,254,1) 50%, rgba(212,254,215,1) 100%)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('dingdanxinxi','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"1px solid #eb8787","cursor":"pointer","padding":"0 16px","margin":"3px 6px 3px 0","outline":"none","color":"#c00","borderRadius":"4px","background":"linear-gradient(240deg, rgba(255,168,153,1) 0%, rgba(247,249,251,1) 50%, rgba(249,156,141,1) 100%)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('dingdanxinxi','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<wentidingdan-cross-add-or-update v-if="wentidingdanCrossAddOrUpdateFlag" :parent="this" ref="wentidingdanCrossaddOrUpdate"></wentidingdan-cross-add-or-update>
...
import wentidingdanCrossAddOrUpdate from "../wentidingdan/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      wentidingdanCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    wentidingdanCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.wentidingdanCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.wentidingdanCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.wentidingdanCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.chengchedian!='' && this.searchForm.chengchedian!=undefined){
            params['chengchedian'] = '%' + this.searchForm.chengchedian + '%'
          }
